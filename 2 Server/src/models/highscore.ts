...
export class Highscore {
    ...
    constructor(name: string, diff: string, score: number) {
        this.userName = Highscore.validUserName(name) ? name.trim() : "Invalid userName";
        this.difficulty = Highscore.validDifficulty(diff) ? diff : "Invalid difficulty";
    ...
    public static validDifficulty (difficulty: string): boolean {
        const validDifficulties = Object.freeze({'Test': 1, 'Beginner': 2, 'Intermediate': 3, 'Expert': 4});
