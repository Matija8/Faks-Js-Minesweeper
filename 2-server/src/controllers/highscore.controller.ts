...
export class HighscoreController {
  ...
  private initRoutes() {
    this.router.get('/', (req: Request, res: Response) =>
      res.send(
        `<a href="./highscores/">/highscores</a> is the route you're looking for!`
      )
    );
    this.router.get('/highscores', HighscoreController.getHighscores);
    this.router.get(
      '/highscores/:difficulty',
      HighscoreController.getHighscoresByDifficulty
    );
  ...
  private static async getHighscores(req: Request, res: Response) {
    let [name, diff]: [string, string] = [
        req.query.userName,
        req.query.difficulty,
      ] as [string, string],
      searchParams: { [k: string]: any } = {};
    let error: string = HighscoreController.setSearchParams(searchParams, [
      name,
      diff,
    ]);
    if (error) {
  ...
  private static async postHighscore(req: Request, res: Response) {
    let [name, diff, score]: [string, string, number] = [
      req.body.userName,
      req.body.difficulty,
      req.body.score,
    ];

    let error: string = HighscoreController.validParams(
      [name, diff, score],
      'POST'
    );
    if (error) {
    ...
    let previousHighscore: any = await mongoService.findOne('highscores', {
      userName: name.trim(),
      difficulty: diff,
    });
    if (previousHighscore === null) {
      ...
      res.send(
        `Added new highscore instance: userName = ${name}, difficulty = ${diff}, score = ${score}`
      );
    } else {
      ...
      if (score < oldScore) {
        ...
        res.send(
          `Updated highscore instance: userName = ${name}, difficulty = ${diff},` +
            ` oldScore = ${oldScore}, newScore = ${score}`
        );
      } else {
        res.send(
          'Previous highscore was better then (or equal to) the new one:\n' +
  ...
  private static async putHighscore(req: Request, res: Response) {
    ...
    /*let result: WriteResult = */ await mongoService.update(
      'highscores', //TODO: check number of updated.
      {
        userName: oldName,
        difficulty: diff,
      },
    ...
    res.send(
      `PUT successful: old userName = ${oldName}, new userName = ${newName}, difficulty = ${diff}, new score = ${newScore}`
    );
  ...
  private static setSearchParams(
    searchParams: { [k: string]: any },
  ...
  private static validParams(
    [name, diff, score]: [string, string, number],
    method: string
  ): string {
